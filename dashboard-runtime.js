(function (win, doc) {
  ...
  // 面板上实际存在的卡片（按 data-source 标记），决定要校验和渲染哪些数据源。
  function cardSlots() {
    var nodes;
    if (!doc || typeof doc.querySelectorAll !== 'function') return [];
    nodes = doc.querySelectorAll('.q-card[data-source]') || [];
    var slots = [];
    var index;
    for (index = 0; index < nodes.length; index += 1) {
      slots.push({ node: nodes[index], name: nodes[index].getAttribute('data-source') });
    }
    return slots;
  }
  ...
  function validPayload(data) {
    ...
    var slots = cardSlots();
    var names = slots.length ? [] : sourceNames;
    for (index = 0; index < slots.length; index += 1) names.push(slots[index].name);
    for (index = 0; index < names.length; index += 1) {
      if (!validSource(names[index], data.sources[names[index]])) return false;
    }
    return true;
  }
  ...
  // 卡片标题用数据里的 label，改配置就能改屏幕上的名字，不用动 HTML。
  function renderCards(data) {
    ...
    for (index = 0; index < slots.length; index += 1) {
      source = data.sources[slots[index].name];
      nameNode = slots[index].node.querySelector('.q-name');
      if (nameNode && source && source.label) ui.textNode(nameNode, source.label);
      if (source && Object.prototype.hasOwnProperty.call(source, 'balance')) {
        updateBalance(source);
      } else {
        updateQuotaCard(slots[index].node.id, source);
      }
    }
  }
